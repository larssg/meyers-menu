...
const DIET_PREFIX_RE = /^(Alm\.?\s*\/?\s*(halal)?\s*:?|Vegetarisk(\/vegansk)?|Vegansk|Halal)\s*:?\s*/i
...
function deref(data: NuxtData, ref: unknown): unknown {
  if (typeof ref !== 'number' || !Number.isInteger(ref) || ref < 0 || ref >= data.length) return undefined
  return data[ref]
}
 ...
 */
function resolveString(data: NuxtData, obj: Record<string, unknown>, property: string): string | null {
  ...
  for (const menuEntryRef of menus) {
    ...
    for (const secRef of sections) {
      ...
      for (const dishRef of dishes) {
        ...
        const title = (resolveString(data, dishNames, 'da') ?? '').replace(WHITESPACE_RE, ' ').trim()
        if (!title) continue
    ...
    mainDishContent = mainDishContent.replace(ALLERGEN_RE, '').trim().replace(/[. ]+$/, '')
