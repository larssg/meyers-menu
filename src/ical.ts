...
export function cleanupTitle(title: string): string {
  ...
  const sections = cleanTitle
    .split(/, Delikatesser:|, Dagens salater:|, Brød:/)
    .filter((s) => s !== '');
  ...
  if (mainSection.length > MAX_TITLE_LENGTH) {
    ...
    if (firstSentence.length > 20 && firstSentence.length < mainSection.length) {
      ...
      mainSection =
        remainingAfterSentence.length > 1 ? `${firstSentence.trim()}...` : firstSentence.trim();
    } else {
      const breakPoint = mainSection.lastIndexOf(
        ' ',
        Math.min(MAX_TITLE_LENGTH, mainSection.length - 1),
      );
      mainSection =
        breakPoint > 40
